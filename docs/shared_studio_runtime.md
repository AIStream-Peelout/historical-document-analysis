...
|---|---|---|
| Kraken/MiDRASH HTR microservice | **Since 2026-09-23 12:26 EDT:** Docker image `kraken-service:k7-blla2026` (kraken 7.0.3 + 2026 blla segmenter baked in), container `kraken-linewise`, **:8002**, `--restart unless-stopped --cpus 6 --memory 10g` (peak 7.0 GiB on 95 MP pages), bind-mount `src/datasets/raw_data/cairo_genizah/custom_model_weights` → `/app/models:ro` | endpoints `/health /preload /transcribe /transcribe_lines`; `/health` reports kraken version + segmenter, and queues behind an in-flight request (use `curl -m 120`). Only ONE kraken *service* container may run. Rollback copy: `kraken-linewise-k4` (image `kraken-service:linewise`, kraken 4.3.13) is kept **stopped with restart=no**; never start it while `kraken-linewise` runs (both bind :8002). Legacy `adoring_fermat` is stopped with restart=no. If :8002 is "already allocated" after a Docker restart, stop the *kraken* duplicate — never a `genizah_search-*` container. Test builds run as `kraken-k7` on :8003 only (`src/services/kraken_microservice/k7/run_k7.sh`). Source: `src/services/kraken_microservice/` (k4) and `…/k7/` (k7); cut-over record `logs/kraken_cutover_0923.json`, results `docs/kraken7_segmenter_ab.md`. |
