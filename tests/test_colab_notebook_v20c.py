# File name: test_colab_notebook_v20c.py
# Date: 8/30/26
# Author: Isaac Godfried. Coded originally by Claude Fable 5.
"""CPU-only validation of the v2.0c genizah Colab notebook (v2.0a data + FULL-weight merger).
...
NB_PATH = _REPO / "src/finetuning/qwen_hebrew/colab/genizah_v20c.ipynb"
...
def test_merger_knob_tri_state(code_cells: List[str]) -> None:
    """MERGER_MODE must be full (the v2.0c arm) and drive all three configs."""
    cell = _find_cell(code_cells, "MERGER_MODE")
    assert 'MERGER_MODE = "full"' in cell, "v2.0c trains the full-weight merger"
...
def test_warm_start_flagship_and_knob_compatibility(code_cells: List[str]) -> None:
    ...
    assert '"isaacmg/qwen3-vl-8b-hebrew-v20c-ckpt"' in trainer_cell
    assert 'run_name="genizah_v20c"' in trainer_cell
