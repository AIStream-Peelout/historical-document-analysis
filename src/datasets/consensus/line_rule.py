...
def assign_fragments(vlm_lines: Sequence[Dict[str, Any]], frags: Sequence[Dict[str, Any]],
                     gap: float = COLUMN_GAP) -> List[List[int]]:
    """Assign Kraken fragments to VLM lines (rule v2: band first, whole column cluster).
    ...
    for i, ln in enumerate(vlm_lines):
        cands = [(j, f) for j, f in enumerate(frags) if overlap_frac(ln["box"], f["box"], 1) >= 0.5]
        if not cands:
            continue
        bx = ln["box"]
        cx = (bx[0] + bx[2]) / 2
